...
bool ProcessorInfoFromOS(int& cpus, int& cores, int& logicalCores, double& clockSpeed)
{
	cpus = 0;
	cores = 0;
	logicalCores = 0;
...
namespace moodycamel
{
	const char* GetCPUString()
	{
		...
		if (buf[0] == 0) {
			::strcpy_s(buf, UNKNOWN_CPU_STRING);
			return buf;
		}
		...
		double clockSpeed;
		if (!ProcessorInfoFromOS(cpus, cores, logicalCores, clockSpeed)) {
			return buf;
		}
		...
		if (cpus > 1) {
			...
			logicalCores /= cpus;
			cores /= cpus;
			::sprintf_s(str, 4," x%d", cpus);
			str += strlen(str);
		}
		ht = ht && logicalCores != cores;
		::sprintf_s(str, 96, " with %d core%s%s @ %.1fGHz%s", cores, cores == 1 ? "" : "s", ht ? " (HyperThreaded)" : "", clockSpeed, cpus > 1 ? " each" : "");
