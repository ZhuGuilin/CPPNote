...
using FallbackGetcpuType = FallbackGetcpu<
    std::conditional_t<kIsMobile, HashingThreadId, SequentialThreadId>>;
...
struct AccessSpreader : private detail::AccessSpreaderBase {
 ...
 public:
  static GlobalState& state() {
    ...
    if ((!state.getcpu.load(std::memory_order_acquire))) [[unlikely]] {
      initialize(state);
    }
    return state;
  }
  ...
  static size_t current(size_t numStripes, const GlobalState& s = state()) {
    ...
    return std::atomic_ref(ref).load(std::memory_order_relaxed);
  }
 ...
 private:
  ...
  ALWAYS_INLINE static CpuCache& cpuCache() {
