...
{
	static constexpr bool eagerRecycle() noexcept
	{
...
/*      IndexedMemPool
        ...
        应用场景	 NumLocalLists_	 LocalListLimit_ |说明
        低并发		4  - 8			50  - 100	 |少量线程，适中缓存
        中等并发		16 - 32			100 - 200	 |通用服务器配置
        高并发		32 - 64			50  - 150	 |减少单一线程囤积过多资源
        内存敏感		8  - 16			20  - 50	 |严格控制本地缓存大小
        极致性能		64 +			200 - 500	 |不惜内存换取最低延迟
*/
template <typename T, uint32_t NumLocalLists_ = 32, uint32_t LocalListLimit_ = 200,
    template <typename> class Atom = std::atomic, typename Traits = IndexedMemPoolTraits<T>>
struct IndexedMemPool
{
    using value_type = T;
    using UniquePtr = std::unique_ptr<T, detail::IndexedMemPoolRecycler<IndexedMemPool>>;
    ...
    static constexpr uint32_t maxIndexForCapacity(uint32_t capacity) noexcept
    {
        // index of std::numeric_limits<uint32_t>::max() is reserved for isAllocated
        // tracking
        return uint32_t(std::min(uint64_t(capacity) + (NumLocalLists - 1) * LocalListLimit,
            uint64_t(std::numeric_limits<uint32_t>::max() - 1)));
    }

    static constexpr uint32_t capacityForMaxIndex(uint32_t maxIndex) noexcept
    {
        ...
        , globalHead_(TaggedPtr{}) 
    {
        ...
        size_t pagesize = size_t(sc_page_size());
    ...
    /// successfully
    uint32_t capacity() noexcept { return capacityForMaxIndex(actualCapacity_); }

    /// Returns the maximum index of elements ever allocated in this pool
    /// including elements that have been recycled.
    uint32_t maxAllocatedIndex() const noexcept
    {
    ...
    void recycleIndex(uint32_t idx) noexcept
    {
    ...
    T& operator[](uint32_t idx) noexcept { return *(slot(idx).elemPtr()); }

    /// Provides access to the pooled element referenced by idx
    const T& operator[](uint32_t idx) const noexcept { return *(slot(idx).elemPtr()); }
    ...
    private:
        ...
        struct Slot 
        {
            //std::aligned_storage<sizeof(T), alignof(T)>::type elemStorage;
			alignas(T) std::byte elemStorage[sizeof(T)];
            ...
            const T* elemPtr() const { return std::launder(reinterpret_cast<const T*>(&elemStorage)); }
        };
        ...
        struct TaggedPtr 
        {
            ...
            uint32_t size() const noexcept { return tagAndSize & SizeMask; }
        ...
        alignas(64) AtomicStruct<TaggedPtr, Atom> globalHead_;

        ///////////// private methods
        uint32_t slotIndex(uint32_t idx) const 
        {
            assert(0 < idx && idx <= actualCapacity_ && idx <= size_.load(std::memory_order_acquire));
            return idx;
        }

        Slot& slot(uint32_t idx) noexcept { return slots_[slotIndex(idx)]; }

        const Slot& slot(uint32_t idx) const noexcept { return slots_[slotIndex(idx)]; }
        ...
        {
            auto stripe = folly::AccessSpreader<Atom>::current(NumLocalLists);
...
class IndexMemoryPool__ : public Observer
{
public:
	...
	void Test() override
	{
		std::print(" ===== IndexMemoryPool Bgein =====\n");
        int ixx = 0;
        if (true || ++ixx)
        {// 这里执行由左向右，满足一个为真后面不会执行
            ;
        }
        IndexedMemPool<TestPool, 6, 100> pool(512);
		auto idx1 = pool.allocIndex();
		pool[idx1].data[0] = 123;
		pool.recycleIndex(idx1);

		std::print(" ===== IndexMemoryPool End =====\n");
	}
};